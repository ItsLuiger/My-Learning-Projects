...
enum enMenuOption { Show = 1, Add = 2, Del = 3, Upd = 4, Find = 5, Transactions = 6 , Exit = 7 };
...
enum enTransactions { enDeposit = 1, enWithdraw = 2, enTotalBalances = 3, enMainMenu = 4 };

sClients ChangeBalance(string AccNum, string PinCode, string Name, string Phone, double newBalance) {

	sClients client;

	client.AccountName = AccNum;
...
bool DepositBalanceToClientByAccountNumber(vector <sClients>& vsClients, string AccountNumber, double Amount , string superator) {
	...
	if (toupper(Ask) == 'Y') {
		...
		for (sClients& C : vsClients) {
			...
			if (C.AccountName == AccountNumber) {
				...
				cout << "\n\nDone Succussfully , new Balance = " << Result;
...
void Deposit() {
	...
	if (SearchClientByAccountNumber(vsClients, AccountNumber, client)) {
		...
		DepositBalanceToClientByAccountNumber(vsClients, AccountNumber, DepositAmount, superator);
...
void Withdraw() {
	...
	if (SearchClientByAccountNumber(vsClients, AccountNumber, client)) {
		...
		DepositBalanceToClientByAccountNumber(vsClients, AccountNumber, Withdraw * -1, superator);
...
void ToTalBalance() {
	...
	cout << "____________________________________________________________________________________________________________________\n\n";
	cout << "|  " << setw(15) << left << "Account Number" << " |  " << setw(40) << left << "Client Name" << " |  " << setw(10) << left << "Balance";
	...
	for (sClients& Client : vsClients) {
		...
		cout << "|  " << setw(15) << left << Client.AccountName << " |  " << setw(40) << left << Client.ClientName << " |  " << setw(10) << left << Client.balance << endl;
	...
	cout << "                                                Totale Balances = " << SumOFBalances;
...
void PerfromTransactionsMenueOption(enTransactions enTransactionsMenueOption) {

	switch (enTransactionsMenueOption) {
...
void ShowTransactionsMenu() {
	...
	PerfromTransactionsMenueOption((enTransactions)UsrSel());
}
